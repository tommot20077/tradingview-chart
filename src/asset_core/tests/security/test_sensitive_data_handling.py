...
class TestSensitiveDataHandling:
    ...
    def test_api_key_masking(self) -> None:
        ...
        def mask_sensitive_data(text: str) -> str:
            ...
            patterns = {
                r"sk-[a-zA-Z0-9]{32}": lambda m: f"sk-{'*' * 28}",
                r"ghp_[a-zA-Z0-9]{36}": lambda m: f"ghp_{'*' * 32}",
                r"Bearer\s+[a-zA-Z0-9\.\-_]{100,}": lambda m: f"Bearer {'*' * 20}...",
                r"password[\':\"\s=]+[^\s\'\"\n]+": lambda m: re.sub(
                ...
                r"[a-f0-9]{40,}": lambda m: "***MASKED***",
                r"-----BEGIN PRIVATE KEY-----.*?-----END PRIVATE KEY-----": lambda m: "-----BEGIN PRIVATE KEY-----\n***MASKED***\n-----END PRIVATE KEY-----",
            }
