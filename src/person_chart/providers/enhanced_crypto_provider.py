...
from ..tools.time_unity import interval_to_seconds
...
class EnhancedInfluxDBManager:
    ...
    def _setup_write_options(self, batch_size: int, flush_interval: int):
        ...
        """
        log.info("正在設定增強型 InfluxDB 寫入選項...")
...
class EnhancedCryptoPriceProviderRealtime(AbstractRealtimeDataProvider, AbstractHistoricalDataProvider):
    ...
    def _handle_binance_message(self, ws, message: str):
        ...
        if price_data.symbol not in self._aggregators:
            self._aggregators[price_data.symbol] = KlineAggregator(
                ...
                target_intervals=config.aggregation_intervals,
    ...
    def start(self):
        ...
        準備好接收實時價格數據。
        """
        log.info("正在啟動 EnhancedCryptoPriceProviderRealtime (增強版)...")
    ...
    def stop(self):
        ...
        """
        log.info("正在停止 EnhancedCryptoPriceProviderRealtime (增強版)...")
    ...
    def get_historical_data(self, symbol: str, interval: str, start: datetime, end: datetime, limit: int, offset: int) -> List[
        Dict[str, Any]]:
        """
        從 InfluxDB 獲取歷史 K 線數據，支持分頁和動態聚合。

        此方法從 InfluxDB 查詢指定交易對在給定時間範圍內的歷史 K 線數據。
        它支持兩種模式：
        1. 如果請求的 `interval` 是預聚合的（即在 `config.aggregation_intervals` 中或為基礎間隔），
           則直接查詢對應的 measurement。
        2. 如果請求的 `interval` 是自定義的且是基礎間隔的倍數，則從基礎數據進行實時聚合。
        查詢結果會被格式化為適合圖表庫使用的字典列表。
        ...
        """
        log.info(f"正在為 {symbol} 獲取從 {start} 到 {end} 的歷史數據 {symbol}@{interval}。")

        is_pre_aggregated = interval in config.aggregation_intervals or interval == config.binance_base_interval
        ...
        try:
            if is_pre_aggregated:
                ...
                base_interval_seconds = interval_to_seconds(config.binance_base_interval)
                target_interval_seconds = interval_to_seconds(interval)

                if target_interval_seconds % base_interval_seconds != 0:
                    raise ValueError(f"自定義間隔 {interval} 必須是基礎間隔 {config.binance_base_interval} 的倍數。")

                df = self.analyzer.query_and_aggregate(symbol, "crypto_price", start, end, f"{target_interval_seconds}s")
