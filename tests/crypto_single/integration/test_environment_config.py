...
class TestEnvironmentSpecificConfiguration:
    ...
    def test_development_environment_configuration_loading(self) -> None:
        """測試開發環境配置載入"""
    ...
    def test_staging_environment_configuration_loading(self) -> None:
        """測試暫存環境配置載入"""
    ...
    def test_production_environment_configuration_loading(self) -> None:
        """測試生產環境配置載入"""
        ...
        with pytest.MonkeyPatch().context() as mp:
            ...
            mp.setenv("CORS_ALLOW_ORIGINS", "https://api.production.com")
    ...
    def test_environment_configuration_switching(self) -> None:
        """測試環境配置切換"""
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setenv("APP_ENVIRONMENT", "production")
            mp.setenv("CORS_ALLOW_ORIGINS", "https://app.example.com")  # Valid CORS for production
            for key, value in base_env_vars.items():
                mp.setenv(key, value)
    ...
    def test_sensitive_information_handling_across_environments(self) -> None:
        """測試敏感資訊在不同環境的處理"""
        # TDD: 定義敏感資訊必須在不同環境中得到適當保護

        sensitive_data = {
            "SECRET_KEY": "super-secret-production-key-with-32-plus-characters",
        ...
        for env in ["development", "staging", "production"]:
            with pytest.MonkeyPatch().context() as mp:
                ...
                if env == "production":
                    ...
                    mp.setenv("DATABASE_URL", "postgresql+asyncpg://user:pass@localhost:5432/staging")
                    mp.setenv("INFLUXDB_HOST", "https://influxdb.staging.com")
                    mp.setenv("INFLUXDB_DATABASE", "crypto_staging")
                    mp.setenv("INFLUXDB_ORG", "crypto-org")
                    mp.setenv("CORS_ALLOW_ORIGINS", "https://staging.example.com")  # Valid CORS for staging
    ...
    def test_environment_variable_priority_order(self) -> None:
        """測試環境變數優先順序"""
        # TDD: 定義環境變數的載入優先順序
    ...
    def test_required_configuration_validation_per_environment(self) -> None:
        """測試各環境必要配置驗證"""
        # TDD: 定義各環境必須有的必要配置驗證
    ...
    def test_development_environment_defaults(self) -> None:
        """測試開發環境預設值"""
    ...
    def test_configuration_inheritance_and_override(self) -> None:
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setenv("APP_ENVIRONMENT", "production")
            mp.setenv("DATABASE_URL", "postgresql+asyncpg://user:pass@localhost:5432/prod")
            mp.setenv("SECRET_KEY", "a-very-long-and-secure-production-secret-key-that-is-at-least-32-characters")
            mp.setenv("INFLUXDB_HOST", "https://influxdb.prod.com")
            mp.setenv("INFLUXDB_TOKEN", "prod-token")
            mp.setenv("INFLUXDB_DATABASE", "crypto_prod")
            mp.setenv("INFLUXDB_ORG", "crypto-org")
    ...
    def test_environment_specific_validation_rules(self) -> None:
        """測試環境特定驗證規則"""
        ...
        with pytest.MonkeyPatch().context() as mp:
            ...
            # 開發環境應該可以接受較弱的配置
            settings = SingleCryptoSettings()
            assert settings.secret_key.get_secret_value() == weak_secret
    ...
    def test_configuration_provides_environment_helper_methods(self) -> None:
        """測試配置提供環境輔助方法"""
