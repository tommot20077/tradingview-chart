...
class MockEventBus(AbstractEventBus, MockImplementationBase):
    ...
    async def wait_for(
        ...
        *,
        ...
        def event_waiter(event: BaseEvent):
...
class TestAbstractEventBusContract(BaseContractTest, AsyncContractTestMixin):
    ...
    async def test_symbol_filtering_mechanism(self, event_bus: MockEventBus):
        ...
        # Define handlers
        def btc_handler(event: BaseEvent):
            btc_events.append(event)

        def eth_handler(event: BaseEvent):
            eth_events.append(event)

        def all_handler(event: BaseEvent):
    ...
    async def test_wait_for_event_interface(self, event_bus: MockEventBus):
        ...
        async def publish_multiple_symbols():
            await asyncio.sleep(0.01)
        ...
        assert btc_event.symbol == "BTCUSDT"
    ...
    async def test_event_type_filtering(self, event_bus: MockEventBus):
        ...
        def trade_handler(event: BaseEvent):
    ...
    async def test_closed_state_handling(self, event_bus: MockEventBus):
        ...
        test_event = MockEvent(EventType.SYSTEM, "BTCUSDT")

        await self.assert_method_raises_when_not_ready(event_bus, "publish", RuntimeError, test_event)
